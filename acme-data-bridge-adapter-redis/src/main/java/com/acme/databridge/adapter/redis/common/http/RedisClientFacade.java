 ...
 */
package com.acme.databridge.adapter.redis.common.http;

import com.acme.databridge.adapter.redis.common.configuration.RedisDataBridgeAdapterOptions;
import com.acme.databridge.adapter.redis.common.configuration.RedisDataBridgeSettings;
import com.acme.databridge.adapter.redis.common.exception.UnsupportedDataBridgeException;
...
public class RedisClientFacade {
  ...
  private final List<RedisDataBridgeSettings> services;

  private final WebClient webClient;

  private final CustomHttpHeader customHttpHeader;

  public RedisClientFacade(WebClient webClient, RedisDataBridgeAdapterOptions configuration) {
  ...
  private void logResponse(Pair<ClientRequest, RedisDataBridgeSettings> request,
  ...
  private Object[] logResponseData(Pair<ClientRequest, RedisDataBridgeSettings> request,
      HttpResponse<Buffer> resp) {
    Object[] data = {
  ...
  private String toUrl(Pair<ClientRequest, RedisDataBridgeSettings> request) {
  ...
  private Pair<ClientRequest, RedisDataBridgeSettings> prepareRequestData(
      FormsAdapterRequest adapterRequest) {
    final Pair<ClientRequest, RedisDataBridgeSettings> serviceData;
    ...
    final Optional<RedisDataBridgeSettings> serviceMetadata = findServiceMetadata(
        serviceRequest.getPath());

    if (serviceMetadata.isPresent()) {
      final RedisDataBridgeSettings metadata = serviceMetadata.get();
      ...
      final String error = String
          .format("No matching service definition for the requested path '%s'",
              serviceRequest.getPath());
      throw new UnsupportedDataBridgeException(error);
    }
    return serviceData;
  }

  private Optional<RedisDataBridgeSettings> findServiceMetadata(String servicePath) {
  ...
  private Single<HttpResponse<Buffer>> callService(
      Pair<ClientRequest, RedisDataBridgeSettings> serviceData, HttpMethod method) {
    final Single<HttpResponse<Buffer>> httpResponse;

    final ClientRequest serviceRequest = serviceData.getLeft();
    final RedisDataBridgeSettings serviceMetadata = serviceData.getRight();
  ...
  private void overrideRequestHeaders(HttpRequest<Buffer> request, RedisDataBridgeSettings metadata) {
  ...
  private void updateRequestQueryParams(HttpRequest<Buffer> request, RedisDataBridgeSettings metadata) {
  ...
  private void updateRequestHeaders(HttpRequest<Buffer> request, ClientRequest serviceRequest,
      RedisDataBridgeSettings serviceMetadata) {
